...
def genSymbolicCommandLineArgs(_state, cmd_nargs, cmd_arglens):
    ...
    return [ _state.solver.BVS('inx{0}'.format(i), max(leni,1) * 8) for (i,leni) in enumerate(cmd_arglens) ]
...
def genInitState(proj, cmd_inxs, std_inx, cmd_ins, std_in):
    ...
    state = proj.factory.entry_state(fs={'/dev/stdin': stdin_file}, args=[''] + cmd_inxs)
...
def solveForInput(proj, cmd_inxs, std_inx, conds):
    ...
    cmd_ins = [ state.solver.eval(cmd_inx, cast_to=str) for cmd_inx in cmd_inxs ] if state.satisfiable() else None
...
def testBinaryWithInput(binary_path, temp_file, cmd_ins, std_in):
    ...
    return_code = subprocess.call(command, stdin=f_stdin, stdout=f_stdout, stderr=f_stdout)
...
def fuzz(proj, binary_path, temp_file, cmd_inxs, std_inx, max_branches, time_limit):
    ...
    while queue:
        ...
        command, return_code = testBinaryWithInput(binary_path, temp_file, cmd_ins, std_in)
        if return_code != 0:
    ...
    return cmd_ins_batch, std_in_batch, return_code_batch, total_paths, time.time() - start

def main(config_file, output_file, temp_file):
  print('config file: {0}, output file: {1}, temp file: {2}'.format(config_file, output_file, temp_file))

  binary_name, cmd_nargs, cmd_arglens, std_len, max_branches, time_limit = readConfig(config_file)
  binary_path = os.path.join(os.path.dirname(config_file), binary_name)

  print("binary: {0}, cmd_nargs: {1}, cmd_arglens: {2}, std_len: {3}, branches: {4}, time: {5}".format(
  ...
  cmd_ins_batch, std_in_batch, return_code_batch, total_paths, time_collapsed = fuzz(proj, binary_path, temp_file, cmd_inxs, std_inx, max_branches, time_limit)

  with open(output_file, 'w') as f:
    #import json
    for cmd_ins, std_in, return_code in zip(cmd_ins_batch, std_in_batch, return_code_batch):
